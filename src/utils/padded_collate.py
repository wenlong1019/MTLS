...
class PaddedBatch:
    ...
    def __init__(self, ids, seq_lengths, targets,
...
def padded_collate(batch):
    ...
    (ids, targets, symb_values, symb_attention_mask, symb_word_starts,
    ...
    padded = PaddedBatch(ids, seq_lengths, targets,
                         symb_values, symb_attention_mask, symb_word_starts,
                         text_input_ids, text_attention_mask, text_word_starts)
...
class PaddedBatchForPretrain:
    """Container class for padded data"""

    def __init__(self, seq_lengths, symb_values, symb_attention_mask, symb_word_starts,
                 text_input_ids, text_attention_mask, text_word_starts):
        self.seq_lengths = seq_lengths
...
def padded_collate_for_pretrain(batch):
    # Sort batch by the longest sequence desc
    batch.sort(key=lambda sequence: len(sequence[5]), reverse=True)
    (symb_values, symb_attention_mask, symb_word_starts,
     text_input_ids, text_attention_mask, text_word_starts) = zip(*batch)
    seq_lengths = torch.LongTensor([len(indices) for indices in symb_word_starts])
    seq_lengths = seq_lengths.clamp(max=len(symb_attention_mask[0]))
    padded = PaddedBatchForPretrain(seq_lengths, symb_values, symb_attention_mask, symb_word_starts,
                                    text_input_ids, text_attention_mask, text_word_starts)
