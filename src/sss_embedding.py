...
class SymbolicEmbedding(nn.Module):
    ...
    def forward(self, symb_values):
        # Get the batch size, number of channels, height, and width of the input
        batch_size, num_channels, height, width = symb_values.shape
        # Check if the input image size matches the model's image size
...
class SelectiveEmbedding(nn.Module):
    def __init__(self, config):
        ...
        # Number of experts to be selected
        self.k = config.num_selected_experts
        # Linear layer to calculate the score of each expert
        self.gate = nn.Linear(config.embed_dim, self.num_experts)
        # Get the experts based on the number of experts and expert dimension
    ...
    def forward(self, E_symb):
        ...
        # Router
        # Reshape the input to have the same size as the expert embeddings
...
class SpatialEmbedding(nn.Module):
    ...
    def forward(self, E_meta):
        E_h = self.encoder(E_meta)
        E_g = self.decoder(E_h, E_meta)
        return E_h, E_g
...
class SSSEmbedding(nn.Module):
    ...
    def forward(self, symb_values, attention_mask=None):
        ...
        E_h, E_g = self.spatial_embedding(E_meta)
