...
def ner_read_col_data(settings, mode):
    # Get the file path from the settings object and mode
    file_path = get_file(settings.data_dir, mode)
    data = pd.read_parquet(file_path)
    labels = data['ner_tags'].tolist()
...
def read_examples_from_file(data_dir, mode):
    # Get the file path for the data directory and mode
